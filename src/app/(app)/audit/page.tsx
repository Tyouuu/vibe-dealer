  ...
  return (
    <>
      <PageHeader
        ...
        meta={<span className="inline-flex items-center gap-1.5 rounded-full border border-ink-800 bg-ink-850 px-2.5 py-1 text-[12px] font-bold text-paper-dim">
            ...
            <tbody>
              {groups.map((group, i) => (
                <Fragment key={`${group.label}-${i}`}>
                  <tr>
                    <td colSpan={7} className="border-b border-ink-800 bg-ink-850 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-paper-dim">
                      {group.label}
                    </td>
                  </tr>
                  {group.events.map((e) => (
