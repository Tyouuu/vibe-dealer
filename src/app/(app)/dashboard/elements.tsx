...
/* ---------------------------------------------------------------- bar list
   ...
   them in place. */
function BarList({ rows }: { rows: { key: string; label: string; value: number; href?: string }[] }) {
...
export function Leaderboard({ rows }: { rows: { id: string; name: string; points: number; href: string }[] }) {
  ...
  return <BarList rows={rows.map((r) => ({ key: r.id, label: r.name, value: r.points, href: r.href }))} />
}
...
export function RegionBars({ rows }: { rows: { region: string; points: number }[] }) {
  ...
  return <BarList rows={rows.map((r) => ({ key: r.region, label: r.region, value: r.points }))} />
}
